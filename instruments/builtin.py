...
``instruments.puma.model`` (guarded by
``tests/test_instrument_registry.py::test_listing_is_lazy_no_mcstas_import``).

To add an instrument: write its plugin module (see ``instruments/puma/plugin.py``
...
from instruments.in12.plugin import IN12_DISPLAY_NAME, IN12_ID, IN12Plugin
...
register(IN12_ID, IN12_DISPLAY_NAME, IN12Plugin)
