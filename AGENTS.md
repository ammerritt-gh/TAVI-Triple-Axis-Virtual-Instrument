...
3. **Test run:** `micromamba run -n tavi-dev python -m pytest tests -q` from the repo root, with `MCSTAS` set to the tavi-dev env's `share\mcstas\resources` (as `run-tavi-dev.bat` does — without it the build-tree tests error out). pytest comes from `requirements-dev.txt`, not `requirements.txt`. Read `tests/README.md` first: no GUI, no McStas compiles/runs in tests. Status 2026-07-26: 672 passed, 1 failed — the only known failure is `tests/test_instrument_packages.py::test_live_packages_are_valid`. (The two `test_controller_is_instrument_agnostic.py` source-scans that failed in the 2026-07-12 status line now pass.)
