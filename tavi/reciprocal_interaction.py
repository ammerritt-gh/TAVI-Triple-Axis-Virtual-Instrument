...
class ReciprocalState:
    ...
    def __post_init__(self):
        ...
        if invalid:
            ...
            p2 = max(choices, key=lambda point: point[0]) if choices else (self.ki, 0.)
...
class ReciprocalInteractionModel:
    ...
    def screen_to_world(self, point, centre):
        ...
        return self._from_plane(plane)
    def world_to_screen(self, point, centre):
        plane=self._to_plane(point); return (centre[0]+self.offset[0]+plane[0]*self.scale, centre[1]+self.offset[1]-plane[1]*self.scale)
    ...
    def drag_p1(self,candidate,*,snap_grid=None,reflections=(),capture=None):
        ...
        if self.locks.delta_e:
            ...
            p2=(q[0]+_unit((p2[0]-start.qx,p2[1]-start.qy))[0]*kf0,q[1]+_unit((p2[0]-start.qx,p2[1]-start.qy))[1]*kf0)
    ...
    def drag_p2(self,candidate):
        ...
        if self.locks.delta_e:
            ...
            p2=self._choose(_intersections((0.,0.),math.hypot(*p2),q,kf0),p2)
            if p2 is None:return DragResult(self.preview,False,"locked kf cannot reach this incident direction")
