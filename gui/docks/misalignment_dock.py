...
This dock provides tools for teachers to create sample misalignment exercises
and for students to practice sample alignment without knowing the true misalignment.
"""
import base64
import struct
...
def encode_misalignment(omega: float, chi: float, psi: float) -> str:
    ...
    Args:
        omega: Misalignment rotation about Y axis (degrees)
        chi: Misalignment tilt about X axis (degrees)
        psi: Additional misalignment rotation about Y (degrees)
    
    Returns:
        A base64-encoded string representing the misalignment.
    """
    # Pack as three floats (24 bytes)
...
def decode_misalignment(hash_str: str) -> tuple:
    ...
    Args:
        hash_str: The encoded misalignment string
    
    Returns:
        Tuple of (omega, chi, psi) in degrees
...
def check_alignment_quality(user_omega: float, user_chi: float, user_psi: float,
    ...
    Args:
        user_omega, user_chi, user_psi: User's current orientation angles
        mis_omega, mis_chi, mis_psi: Hidden misalignment angles
        tolerance_good: Degrees within which alignment is considered "good"
        tolerance_close: Degrees within which alignment is considered "close"
    
    Returns:
        Dictionary with alignment status for each axis and overall status.
    """
    # The goal is to have user angles cancel out the misalignment
    # Effective total should be ~0 for perfect alignment
    
    # For omega/psi combined effect on Y-axis rotation
    y_axis_error = abs(user_omega + user_psi - (-mis_omega - mis_psi))
    # For chi effect on X-axis tilt
...
class MisalignmentDock(QDockWidget):
    """Dock widget for sample misalignment training exercises."""
    
    def __init__(self, parent=None):
        super().__init__("Misalignment Training", parent)
        ...
        # Teacher section: Generate misalignment
        ...
        # Student section: Load misalignment
